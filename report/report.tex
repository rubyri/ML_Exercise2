...
Worrying about the tree fitting to noise we first limited the growht of the tree by its paramters 
(\texttt{max_depth=15, min_samples_split=5}). However we archieved the worst model performance 
yet, so in a second run, we removed all depth restrictions (\texttt{max_depth=None}).
...
\begin{table}[H]
\centering
\caption{Cross-validation results for the Decision Tree on full and reduced feature sets.}
...
With over 10,000 discrete features and only 750 samples, most features contribute weakly and 
redundantly to the prediction. Even after dimensionality reduction, the ratio 
